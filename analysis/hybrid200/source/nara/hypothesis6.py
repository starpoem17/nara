"""Frozen hypothesis-6 baseline rules. Never imports labels, retrieval, or a model.
...
def eligibility_spans(text):
    ...
    for offset,line,flat in lines:
        # Eligibility-like references inside long prose are not headings.
        eligible=len(flat)<=90 and ELIGIBILITY.search(flat) is not None
        numbered=NUMBERED.match(flat)
        if start is not None:
            end=(eligible or STOP.search(flat) is not None
