...
`scripts/recover_prefix_batch11.py` preserves first_pass artifacts and all failed/recovery model events. `scripts/summarize_prefix_batch11.py` checks same source/input/rules,200×24 completeness,49-column CSV, token totals,1→11 submissions and unchanged originally successful records. Prefix unit tests3 passed. Single historical comparison; no repeat/generalization claim. Current result does not support replacing batch8 when Macro preservation is the priority.
