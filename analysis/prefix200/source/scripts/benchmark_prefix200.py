...
def main():
    ...
    manifest={'records':200,'groups':groups,'limits':asdict(limits),'thinking':False,'max_model_len':32768,
              'source_sha256':hashes,'counts':counts,'baseline':'analysis/compact200/groups12_off_h6',
    ...
    # Same engine configuration as the frozen historical experiment; OFF template/sampling.
    ...
    report={'name':'source_first_groups12_off_h6','options':{'model_dir':MODEL,'max_model_len':32768,'thinking':False,'item_group_size':max(map(len,groups))},
            'limits':asdict(limits),'groups':groups,'records':200,'gpu':torch.cuda.get_device_name(0),
