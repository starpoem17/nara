...
def main():
    ...
    for schedule in schedules:
        ...
        report={'name':name,'records':len(records),'schedule':schedule,'groups':groups,'engine':engine,
            'prediction_seconds':elapsed,'load_seconds':load,'total_seconds':elapsed+load,
            'options':{'thinking':'mixed' if args.kind=='mixed' else True,'item_group_size':6},'limits':asdict(limits),
