...
from scripts.evaluate_dev import evaluate,read_csv,ITEMS
...
def main():
    ...
    for name in ['main','optimized','optimized_off']:
        ...
        if 'recovery' in report:
            ...
            assert hashlib.sha256(Path('scripts/recover_hybrid200.py').read_bytes()).hexdigest()==digest
    ...
    assert hashlib.sha256(Path(batch_plan['source']).read_bytes()).hexdigest()==batch_plan['sha256']
