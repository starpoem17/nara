...
```bash
uv run --locked python -m nara.legal_index build
```
...
`script.py`의 `--input`, `--limit`, `--thinking` 옵션은 이 명령에 적용되지 않습니다.
...
| 변경 대상 | 파일 |
|---|---|
| 기본 실험의 공통 지시문·항목별 판단 기준 | `nara/compact_criteria.json`의 `common`, `items` |
| 원문과 판단 기준의 배치 | `nara/compact_predictor.py`, `nara/prefix_predictor.py` |
| 12그룹 구성과 v2·v3 제외 | `scripts/benchmark_grouping_time.py`의 `PLANS`, `scripts/benchmark_compact200.py`의 `configuration` |
| v2·v3 규칙 | `nara/hypothesis6.py` |
| 공고 간 실행 순서·동시 처리 | `nara/prefix_pipeline.py`, `scripts/benchmark_prefix_pipeline.py` |
| 모델 실행·thinking 예산·캐시·계측 | `nara/vllm_model.py` |
| 실패 복구·기록 집계 | `nara/recovery.py` |
| `script.py`의 RAG 프롬프트 | `nara/prompt.txt` (`--legal-criteria` 사용 시 `nara/legal_criteria.json` 추가) |

`nara/prompt.txt`를 수정해도 기본 12그룹 실험의 프롬프트는 바뀌지 않습니다.
...
- 실패 복구를 `nara/recovery.py`로 통합했습니다. 저장된 GPU 결과 재생과 복구 시뮬레이션으로
  검증했으며, 이 변경에 대한 새 GPU 시간·F1 측정은 없습니다. [복구 검증](<../analysis/recovery_refactor/validation.json>).

## RAG 비교 실행·제출 진입점

동적 RAG 베이스라인과 제출 진입점은 프로젝트 루트의 `script.py`입니다.
...
model/legal_index입니다. `script.py`는 `PPS_DATA_DIR`, `PPS_OUTPUT_DIR`, `PPS_MODEL_DIR`, `PPS_EMBED_DIR`를 읽으며,
...
`script.py` 실행·패키징 시 프롬프트를 포함한 `nara/`도 함께 준비합니다.
