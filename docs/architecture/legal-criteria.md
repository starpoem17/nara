...
Assets: [criteria JSON](../../src/inference/legal_criteria.json), [human review](../reports/analysis/feature_legal_criteria_v1.md), [full system prompt](../../analysis/legal_criteria_v1_system_prompt.txt).
...
Run `uv run --locked python -m nara.tools.check_legal_criteria`, then `uv run --locked python -m nara.tools.render_legal_criteria` after changes. The latter generates the human review from the same JSON plus the preflight result.

[Measured report](../../analysis/legal_criteria_v1_context.json): thinking template, 200 dev notices, 32768 context, initial prompts only, reserve output +128. With output 2048, group sizes 24/6/1 fail 22/5/0 notices; with output 4096 they fail 28/15/7. Maximum inputs: 35761/31714/30553. Group size 1 requires 4800 initial tasks and leaves minimum 39 tokens with output 2048. Retrieval/retries are not certified by this check. Compress criteria or design long-document handling before treating the enriched prompt as a full-run configuration.
