...
Run `uv run --locked python scripts/check_legal_criteria.py`, then `uv run --locked python scripts/render_legal_criteria.py` after changes. The latter generates the human review from the same JSON plus the preflight result.
