...
- Writes `trace.jsonl` and `report.json` for prediction failures; atomically writes `submission.csv` only if all records succeed. Package `nara/` including `prompt.txt`, plus `model/legal_index/` assets.
