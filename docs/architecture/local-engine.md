...
Implementation: `nara/vllm_model.py`. Default caller: `scripts/benchmark_prefix_pipeline.py` via `scripts/run_experiment.py`. Scheduling remains in `nara/prefix_pipeline.py`; conversation/retrieval/validation remain in `nara/inference.py` and `nara/continuous.py`.
