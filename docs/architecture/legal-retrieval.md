...
Code: [builder](../../nara/legal_index.py), [retriever](../../nara/retrieval.py). Caller: [dynamic RAG](dynamic-rag.md).
...
```bash
OMP_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 uv run --locked python -m nara.legal_index build \
...
```bash
OMP_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 uv run --locked python -m nara.legal_index search \
