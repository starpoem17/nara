...
5. Keep active utilities in `scripts/`, inference implementation in `nara/`. Check imports, source-copy lists, CLI bootstrap paths and saved hashes before moving code.
...
7. `scripts/jsonl_to_md.py` renders source records, not handover notes. Its explicit data output path remains supported. Preserve `user/`, original distribution data and model/index assets.
8. Ignore local raw traces/caches as configured by `.gitignore`; tracked reports may reference local-only evidence. Such a link identifies required local material, not a guarantee that another clone contains it.
