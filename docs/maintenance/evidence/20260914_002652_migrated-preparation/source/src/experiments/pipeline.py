...
def execute(out, grouping="groups12", *, prepare_only=False, smoke_only=False):
    out = Path(out)
    retained = Run.find(out)
    phase = "preparation" if prepare_only else "smoke" if smoke_only else "full200"
    retained.require_active_attempt(out, {"grouping": grouping, "phase": phase, "output_tokens": 512, "thinking": False, "seed": 0})
    if any(out.iterdir()):
