"""Dynamic RAG baseline for local experiments."""
import argparse
...
def main():
    ...
    parser.add_argument("--legal-criteria", action="store_true",
                        help="Include supplied-snapshot legal criteria; preflight the 32K context budget")
    ...
    predictor = Predictor(model, retriever, table, schema, limits=limits,
                          legal_criteria=args.legal_criteria)
    keys = list(table)
