...
def compare(baseline, candidate):
    ...
    retained.capture_input("data/dev_labels.csv")
    ...
    if hashlib.sha256(Path('data/dev_labels.csv').read_bytes()).hexdigest() != a['sha256']['data/dev_labels.csv']:
    ...
    lines += ['', 'This exploratory comparison uses the same dev200. It is neither holdout validation nor a significance test.']
