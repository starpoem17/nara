...
# A deadline for holding qualifications is body text, not a submission heading.
DEADLINE_REQUIREMENT = re.compile(r'^' + SECTION_PREFIX + r'(?:입찰서|제안서)제출마감일.*(?:까지|이전).*(?:자격|갖추|구비|등록|소지|업체)')
...
def numbered_level(match):
    """Keep Arabic subclauses under Roman sections, and 1) clauses under 3."""
    if match[0].endswith('장') or not match[1].removeprefix('제').isdigit():
        return 0
    return 2 if match[0].endswith(')') else 1
...
def eligibility_spans(text):
    ...
    for offset,line,flat in lines:
        ...
        if start is not None:
            stop_heading = (STOP.search(flat) is not None
                            and DEADLINE_REQUIREMENT.match(flat) is None)
            next_section = (numbered is not None and heading_number is not None
                            and (numbered_level(numbered)<numbered_level(heading_number)
                                 or (numbered_level(numbered)==numbered_level(heading_number)
                                     and numbered[1]!=heading_number[1])))
            end=(eligible or excluded_section or stop_heading or next_section)
            if end:
                spans.append((start,offset))
                start=None
        if eligible:
            start=offset
            heading_number=numbered
