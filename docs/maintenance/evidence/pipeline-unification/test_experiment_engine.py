...
from nara.inference import pipeline as runner
...
class ExperimentEngineTests(unittest.TestCase):
    ...
    def test_arbitrary_notice_count_uses_the_same_recovery_pipeline(self):
        self.run_recovery(True, False, count=3)

    def run_recovery(self, isolated_needed, isolated_fails, count=200):
        def judgments():
        ...
        records = [{'id': str(i), 'docs': [{'doc_id': 'D0', 'type': '공고문',
                    ...
                    'meta': {'낙찰방법': '협상에의한계약'}} for i in range(count)]
        ...
        with tempfile.TemporaryDirectory() as temp, ExitStack() as stack:
            retained = Run.create("scheduling", "test", "Recovery accounting", {"main": runner.settings("groups12", input_path="data/dev.jsonl")}, root=Path(temp))
            ...
            with redirect_stdout(io.StringIO()), (self.assertRaisesRegex(RuntimeError, "Unresolved") if isolated_fails else nullcontext()), retained.condition("main") as out:
                runner.execute(out, input_path="data/dev.jsonl")
            ...
            expected = count + (2 if isolated_needed else 1)
            attempts = [json.loads(s) for s in (out / 'recovery_attempts.jsonl').read_text().splitlines()]
            self.assertEqual(len(attempts), expected - count)
            self.assertEqual(bool(attempts[0]['trace'][0]['error']), isolated_needed)
            self.assertEqual(sum(len(a['requests']) for a in attempts), expected - count)
            ...
            self.assertEqual([s['attempt'] for s in scheduler[-(expected - count):]],
                             ['recovery', 'isolated'] if isolated_needed else ['recovery'])
            self.assertTrue(all(s['stage'] == 'recovery' for s in scheduler[count:]))
