...
def main():
    ...
    predictor = SourceFirstPredictor(counter, None, table, schema, limits=limits)
    ...
    manifest = dict(groups=[KEYS], original_group_number=4, records=200, thinking=False,
    ...
    sources = list(Path('nara').glob('*.py')) + [Path('nara/compact_criteria.json'), Path(__file__),
              Path('script.py'), Path('data/dev.jsonl'), Path('data/項目表.json') if False else Path('data/항목표.json'),
    ...
    model = VLLMModel(MODEL, thinking=True, max_num_seqs=16, max_num_batched_tokens=8192, enable_chunked_prefill=True)
