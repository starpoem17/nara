...
class Run:
    ...
    def create(cls, area, summary, question, conditions, *, root=None,
        ...
        if prior:
            ...
            comparison = {"unchanged": [k for k in conditions if k in before and conditions[k] == before[k]],
    ...
    def save(self):
        ...
        for kind in ("predecessors", "followups"):
            for relation in m.get(kind, []):
                ...
                lines += ["", f"{kind.title()}: [{relation['run_id']}](<{relative_link(linked.report, self.report)}>) — {relation['reason']}. Conditions: `{json.dumps(relation.get('condition_changes', {}))}`"]
