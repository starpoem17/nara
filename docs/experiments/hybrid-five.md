...
- `src/experiments/benchmark_hybrid200.py`: real main200 (per notice OFF seed1, cached OFF3, ON1), isolated same-six OFF200/ON256200, cache24 matched pairs.
- `src/experiments/benchmark_hybrid_batch_controls.py`: same-six ON1024 batch8; real optimized5 mixed ON256 batch8; real optimized5 allOFF (seed2, cached8).
- `src/evaluation/summarize_hybrid200.py`: audits, composite controls, paired notice bootstrap2000, all24 TP/FP/FN/F1, timing and cache summaries.
