...
Completed 2026-09-11, RTX5090. Report: [comparison](<compact200.md>). Artifacts: `analysis/compact200/` (`comparison.json`, `validation.json`). Full200 in all6 runs after bounded recovery. No source clipping. `user/` unchanged.
...
Validation:35 pre-run unit tests; `experiments/legacy_compact_baseline/code/validate_compact200.py` passed all6 complete200 ID/24bit trace/CSV checks, source/frozen-rule hashes, actual context/output budgets, modes, H6 rule identity and prompt/schema isolation. All emitted evidence exact-source valid, but many missing nonabsence positive quotes (see comparison); exact substring is not semantic proof. Evaluation includes FP/FN/evidence errors. `experiments/legacy_compact_baseline/code/summarize_compact200.py` produces report and direct-rule counterfactual. No hidden test run or L40S measurement.

[Original aggregate evidence](<../../maintenance/evidence/structure-migration/original_reports/docs/reports/analysis/compact200/comparison.md>); historical measurements are preserved as source material.
