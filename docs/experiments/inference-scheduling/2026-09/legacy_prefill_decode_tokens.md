# Does batching affect tokens at a fixed shared output history?

Current judgment: **Selected-divergence token probe; phase sensitivity observed, specific kernel/layer attribution unverified.**

Reason and scope: Stage1 output divergence selected the first four cases by input order; stage2 appended common generated prefixes and used unconstrained token probes with prefill/decode controls. Historical local development evidence; no unseen-data or competition-server generalization is established.

## Current interpretation and material history

The first four divergent cases by input order supplied common generated prefixes; no label-based selection. Prefill solo versus8 changed selected tokens3/4; decode background injection preserved first-token scores then changed shared-history scores from the fourth token in4/4. All16 repeated same-condition pairs matched. Probes used unconstrained decoding, not production output masks; no full200 quality claim.

## Run identity and conditions

Run ID: `legacy_prefill_decode_tokens`. Historical start time: **unknown**. The available metadata does not establish an orchestration start; initialization/log/preparation timestamps were not substituted. Execution month: September 2026, supported by the original project records and original run names.
...
Selected-divergence token probe; phase sensitivity observed, specific kernel/layer attribution unverified.
...
[Retained execution directory](<../../../../experiments/legacy_prefill_decode_tokens>) · [Execution metadata and original-file hashes](<../../../../experiments/legacy_prefill_decode_tokens/run.json>)
...
[legacy_prefill_decode_probe](<legacy_prefill_decode_probe.md>): Stage1 output divergence selected the first four cases by input order; stage2 appended common generated prefixes and used unconstrained token probes with prefill/decode controls.
...
- [code](<../../../../experiments/legacy_prefill_decode_tokens/code>): retained source/evidence; execution snapshots and migration-time helpers are distinguished.
