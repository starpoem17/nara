...
Artifact base: `analysis/hybrid200/`; unqualified JSON/CSV/log/source paths below use this base. Detailed Markdown reports: [index](<../README.md>).

Completed 2026-09-11. All variants200×24, no remaining failures or context overflow. Canonical report: `docs/experiments/inference-scheduling/2026-09/legacy_hybrid200.md`; detailed scores `per_feature.csv`, `comparison.json`; timing `timing_analysis.json`, `cache_analysis.json`; audit `validation.json`.
...
- `experiments/legacy_hybrid200/code/benchmark_hybrid200.py`: real main200 (per notice OFF seed1, cached OFF3, ON1), isolated same-six OFF200/ON256200, cache24 matched pairs.
- `experiments/legacy_hybrid200/code/benchmark_hybrid_batch_controls.py`: same-six ON1024 batch8; real optimized5 mixed ON256 batch8; real optimized5 allOFF (seed2, cached8).
- `experiments/legacy_hybrid200/code/summarize_hybrid200.py`: audits, composite controls, paired notice bootstrap2000, all24 TP/FP/FN/F1, timing and cache summaries.
...
Historical baselines: [prefix12](<prefix-cache.md>), [ungrouped ON H6](<../prompt-design/compact200.md>). Same-group v10–v13 changed27/800 bits vs prefix12 (v10:18,v11:9); grouping effects not causally isolated from batching/numerical paths. Original ON/OFF token templates differ at front, so source KV cannot cross modes.

Time anchor: [colleague](<../colleague-hypotheses/colleague-submission.md>) gives provisional local200 inference target533.12s; hardware/quantization/batching dependent, not a competition passing guarantee.
