...
Reproduce: `uv run --locked python scripts/benchmark_grouping_time.py --output NEW_PATH`, then `uv run --locked python scripts/summarize_grouping_time.py --input NEW_PATH`. Initial prompts, selected input, source snapshot and per-task traces are saved under each output directory. Trace/report/CSV IDs and token counts were cross-checked; [validation](../../analysis/grouping_time32/validation.json).
...
- Reproduce evaluation: `uv run --locked python scripts/evaluate_grouping_effect.py`, then regenerate timing summary. Raw benchmark report and traces remain unchanged.
