    ...
    def generate(self, turns):
        ...
        params = [
            SamplingParams(
                ...
                thinking_token_budget=(
                    min(256, turn.max_tokens // 4)
                    if turn.schema.get("properties", {}).get("action", {}).get("const") == "search"
                    else min(1024, turn.max_tokens // 2)
        ...
        for turn, output, param in zip(turns, outputs, params):
            ...
            result[turn.task_id] = Reply(
                ...
                thinking_tokens=self.count_text(parsed["thinking"] or ""),
                thinking_budget=param.thinking_token_budget)
        return result
