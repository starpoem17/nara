"""Extended output-budget repair after the standard short-evidence repair fails."""
import argparse
...
from nara.inference import Limits,Prediction,compact
...
POLICY={'output_tokens':4096,'thinking_budget':1024,'search_rounds':0,'retries':1,
...
def main():
    ...
    for p,h in manifest['source_sha256'].items():
        assert hashlib.sha256(Path(p).read_bytes()).hexdigest()==h, f'Changed original source {p}'
    plans=[]
    for name in args.names:
        folder=root/name
        report=json.loads((folder/'report.json').read_text())
        if report['failed_ids']:
            if 'extended_recovery' in report:raise ValueError(f'Already repaired {name}; inspect remaining failure')
            plans.append((folder,report))
    if not plans:
        print('No failed notices to repair',flush=True);return
    (root/'extended_recovery_policy.json').write_text(json.dumps(POLICY,indent=2)+'\n')
    dest=root/'source/scripts/recover_compact_extended.py';dest.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(__file__,dest)
    ...
    for folder,report in plans:
        saved=folder/'before_extended_recovery';saved.mkdir()
        ...
        counter=RepairPredictor(model,None,t,s)
        largest=max(model.count_messages(counter._messages(r,g)) for r in failed for g in groups)
        output_tokens=min(4096,32768-largest-128)
        assert output_tokens>2048, 'No additional output room without changing source'
        predictor=RepairPredictor(model,None,t,s,limits=Limits(search_rounds=0,output_tokens=output_tokens))
        for i,g in enumerate(groups):
            (folder/f'extended_recovery_prompt_{i+1:02}.txt').write_text(predictor._messages(failed[0],g)[0]['content']+'\n')
            (folder/f'extended_recovery_schema_{i+1:02}.json').write_text(json.dumps(predictor._schema(g,False),ensure_ascii=False,indent=2)+'\n')
        ...
        for original in originals:
            if original.record_id in replacements:
                ...
                for task in new.trace:
                    task['task_id']='extended_recovery:'+task['task_id']
                    task['attempt']='extended_recovery'
        ...
        report['extended_recovery']={'policy':POLICY,'actual_output_tokens':output_tokens,'prediction_seconds':seconds,'separate_process_load_seconds':load,
