...
for budget in [8192,2048,16384]:
    with (ROOT/f'pilot_{budget}.log').open('w') as log:
        subprocess.run([sys.executable,'scripts/benchmark_continuous.py','--kind','pilot','--tokens',str(budget)],stdout=log,stderr=subprocess.STDOUT,check=True)
...
for kind in ['six','mixed']:
    with (ROOT/f'{kind}_{winner}.log').open('w') as log:
        subprocess.run([sys.executable,'scripts/benchmark_continuous.py','--kind',kind,'--tokens',str(winner)],stdout=log,stderr=subprocess.STDOUT,check=True)
