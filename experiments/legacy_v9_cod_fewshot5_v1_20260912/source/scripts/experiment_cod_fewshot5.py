...
from scripts.experiment_v9_group import RecordedModel,MODEL,KEYS,dump
...
def main():
    ...
    manifest={'seed':20260912,'ids':[r['id'] for r in records],'group':KEYS,'thinking':True,'thinking_budget':256,'limits':asdict(limits),'prompt_tokens':counter.count_text(instruction),'input_tokens':[counter.count_messages(m) for m in messages],
    ...
    hashes={}
    for p in [Path(__file__),Path('scripts/experiment_v9_group.py'),*Path('nara').glob('*.py'),Path('nara/compact_criteria.json'),Path('data/dev.jsonl')]:
