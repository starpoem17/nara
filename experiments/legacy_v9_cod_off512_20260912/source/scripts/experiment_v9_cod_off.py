...
from nara.inference import Limits, Turn, Reply
...
class RecordedModel(VLLMModel):
    def generate(self, turns):
        ...
        for turn,out in zip(turns,outputs):
            ...
            reply=Reply(answer.strip(),c.finish_reason,len(out.prompt_token_ids),len(ids))
...
def main():
    ...
    sources = [Path('analysis/v9_cod_fewshot5/prompt_v1.txt')] + list(Path('nara').glob('*.py')) + [Path('nara/compact_criteria.json'), Path(__file__),
