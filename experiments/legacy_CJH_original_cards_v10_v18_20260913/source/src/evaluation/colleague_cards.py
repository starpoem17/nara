...
ROOT = Path(__file__).resolve().parents[2] if Path(__file__).parent.name == "evaluation" else Path(__file__).resolve().parents[1]
from nara.experiments.colleague_cards import FEATURES, PARSING, RUNS, dump, read_jsonl, sha
...
def _report(author, out, evaluation, rows, cases, errors, bundle):
    ...
    text += ["", "```bash", "uv run --locked python -m nara.evaluation.colleague_cards", "```", ""]
