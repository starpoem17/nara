...
parts += ['## 해석상 남은 한계',
          ...
          '## 프롬프트 연결과 32K 검사',
          '`src/cli.py --legal-criteria`를 지정하면 공통 기준과 현재 판단할 항목의 상세 기준·조문을 시스템 프롬프트에 추가한다. 기존 기본 프롬프트는 유지한다. 항목 분할은 기존 `--item-group-size` 옵션을 사용하며 자동 분할·원문 잘라내기는 하지 않는다.',
...
parts += ['', '1항목씩/출력 2,048 설정은 초기 입력이 전부 들어가지만 최대 입력의 추가 여유가 39토큰뿐이며 호출 수가 4,800개다. 현재 thinking 기본 출력 4,096에서는 이 설정도 7건이 넘는다. 따라서 다음 실험 전에 기준 압축 또는 긴 문서 처리 방식을 정해야 한다. 위 호출 작업 수는 초기 작업 기준이며 검색·재시도는 추가 호출이다.',
          ...
          '재생성: `uv run --locked python src/tools/check_legal_criteria.py` 후 `uv run --locked python src/tools/render_legal_criteria.py`.',
          '파일: [판단 기준 JSON](../src/inference/legal_criteria.json), [실제 전체 시스템 프롬프트](legal_criteria_v1_system_prompt.txt), [200건 토큰 검사](legal_criteria_v1_context.json).']
