...
from nara.experiments.config import MODEL, PLANS, configuration
...
def preflight(out, records, table, schema, limits):
    ...
    manifest = {'records': len(records), 'ids': [r['id'] for r in records], 'configurations': configs,
                ...
                'compression': 'English concise criteria in src/inference/compact_criteria.json; full documents intact',
                'source_sha256': {str(p): hashlib.sha256(p.read_bytes()).hexdigest() for p in
                   [Path('src/__init__.py'), Path('src/runtime.py'), Path('src/records.py'), Path('src/paths.py'),
                    Path('src/experiments/__init__.py'), Path('src/experiments/config.py'),
                    Path('src/inference/__init__.py'), Path('src/retrieval/__init__.py'),
                    Path('src/rules/__init__.py'), Path('src/evaluation/__init__.py'),
                    Path('src/experiments/benchmark_compact200.py'), Path('src/inference/compact_criteria.json'), Path('src/inference/compact_predictor.py'),
                    Path('src/inference/predictor.py'), Path('src/inference/conversation.py'), Path('src/inference/engine.py'), Path('src/inference/legal_criteria.json'),
    ...
    if args.stage=='baseline':
        if (out/'manifest.json').exists() and not args.prepare_only:
            ...
            for p,h in manifest['source_sha256'].items():
                assert hashlib.sha256(source_path(p).read_bytes()).hexdigest()==h, f'Changed source {p}'
        else:
        ...
        p=Path('src/rules/qualification.py'); dest=out/'source'/p;dest.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(p,dest)
